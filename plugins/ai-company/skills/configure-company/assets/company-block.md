...
### 총괄 절차
1. **시작·재정박**: `SESSION.md` → `직원명부.json` → `agentlayer task list` 순으로 읽는다. Monitor 도구로 `agentlayer task watch {INBOX}`를 `persistent: true`로 띄운다(상주 수신 — 이벤트가 한 줄 JSON으로 온다). `agentlayer board`로 보드를 본다(디스코드 카드에도 "업무 보드" 절이 있다).
...
5. **마감**: `agentlayer task done <업무ID>`(task.md를 done으로 닫고 자식 업무의 READY 이벤트를 보낸다), `log.md`에 한 줄, 결과물을 `결과물/<업무ID>/`에 복사·링크.
