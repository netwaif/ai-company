...
3. **배정**: 명부에서 직원을 고른다.
   - Claude 봇 직원: `bot-thread open <bot> <channel_id> <업무ID>` → 스레드 ID 출력 → `bot-thread ensure <bot> <스레드ID>`(창·세션 생성, 준비까지 최대 60초; 출력은 에이전트 이름 `<bot>-t<끝6자리>`이며 창 이름이 아님) → 창 이름은 `t<스레드ID 끝 6자리>`. `agentlayer task assign <업무ID> <session>:t<끝6자리> --inbox {INBOX}` 뒤 `agentlayer send <session>:t<끝6자리> - < 업무요청/<업무ID>.md` (파일 경로 대신 본문을 보낸다 — 직원 폴더 밖 읽기 권한 프롬프트 회피).
   - Codex·Gemini 봇 직원: 스레드 없이 `agentlayer task assign <업무ID> <session> --inbox {INBOX}` → `agentlayer send <session> - < 업무요청/<업무ID>.md`. 세션에 pane이 둘 이상이면 agentlayer가 창 명시를 요구한다 — `agentlayer status`로 창 이름을 확인해 `<session>:<창>`으로 보낸다.
